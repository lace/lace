...
def _load(f, mesh=None):
    ...
    import plyutils # FIXME pylint: disable=relative-import
...
def _dump(f, mesh, flip_faces=False, ascii=False, little_endian=True, comments=[]):
    ...
    plyutils.write(list([list(x) for x in mesh.v]),
                   ...
                   list([list((x*255).astype(int)) for x in (mesh.vc if mesh.vc is not None else [])]), # FIXME pylint: disable=superfluous-parens
                   f.name, ascii, little_endian, list(comments),
                   list([list(x) for x in (mesh.vn if mesh.vn is not None else [])])) # FIXME pylint: disable=superfluous-parens
