...
class TestArcBall(unittest.TestCase):
    def test_ArcBall(self):
        ...
        ThisRot = Matrix3fSetRotationFromQuat4f (ThisQuat) # FIXME pylint: disable=bad-whitespace
        # Linear Algebra matrix multiplication A = old, B = New : C = A * B
        ThisRot = Matrix3fMulMatrix3f(LastRot, ThisRot)
        Transform = Matrix4fSetRotationFromMatrix3f (Transform, ThisRot) # FIXME pylint: disable=bad-whitespace
        np.testing.assert_array_almost_equal(Transform, [[0.97764552, -0.1380603, 0.15858325, 0.], [0.10925253, 0.97796899, 0.17787792, 0.], [-0.17964739, -0.15657592, 0.97119039, 0.], [ 0., 0., 0., 1.]]) # FIXME pylint: disable=bad-whitespace
        # Done with first drag

        # second click
        LastRot = copy.copy(ThisRot)
        np.testing.assert_array_almost_equal(LastRot, [[0.97764552, -0.1380603, 0.15858325], [0.10925253, 0.97796899, 0.17787792], [-0.17964739, -0.15657592, 0.97119039]])
        mouse_pt = Point2fT(350,260) # FIXME pylint: disable=bad-whitespace
        ArcBall.click(mouse_pt)
        # second drag
        ...
        np.testing.assert_array_almost_equal(Transform, [[ 0.88022292, -0.08322023, -0.46720669, 0.], [0.14910145, 0.98314685, 0.10578787, 0.], [0.45052907, -0.16277808, 0.8777966, 0.], [0., 0., 0., 1.00000001]]) # FIXME pylint: disable=bad-whitespace
