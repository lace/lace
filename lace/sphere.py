...
class Sphere(Mesh):
    ...
    def v(self, val):
        if val is not None:
            raise ValueError("You can't change the verts of a sphere directly, change the center and radius instead")

    def __str__(self):
        return "sphere of radius %s centered at %s" % (self.radius, self.center)
    ...
    def intersection_vol(self, sphere):
        ...
        R, r = (self.radius, sphere.radius) if (self.radius > sphere.radius) \
